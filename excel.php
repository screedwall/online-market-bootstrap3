...
$connection = new mysqli("localhost", "root", "mysql", "site");
// Выбираем кодировку UTF-8
$connection->set_charset("utf8");

// Загружаем файл Excel
$PHPExcel_file = PHPExcel_IOFactory::load("./cats.xlsx");
...
$connection->query("CREATE TABLE " . $table_name . " (" . str_replace(",", " TEXT NOT NULL,", $columns_str) . " TEXT NOT NULL)"); 
        // Количество строк на листе Excel
        $rows_count = $worksheet->getHighestRow();
...
//echo excel2mysql($PHPExcel_file->getActiveSheet(), $connection, "items", 1) ? "OK\n" : "FAIL\n";

// Перебираем все листы Excel и преобразуем в таблицу MySQL
/*foreach ($PHPExcel_file->getWorksheetIterator() as $index => $worksheet) {
  echo excel2mysql($worksheet, $connection, "items" . ($index != 0 ? $index : ""), 1) ? "OK\n" : "FAIL\n";
}*/
?>
